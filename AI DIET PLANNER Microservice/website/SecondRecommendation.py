    ...
    if request.user.is_authenticated:
        class Recommender:
            ...
            def find_neighbors(self, dataframe, features, k):
                ...
                similar_neighbors = self.k_neighbor(
                    [final_input], features_df, dataframe, k)
            ...
            def user_based(self, features, user_id):
                ...
                similar_users = self.find_neighbors(
                    self.profiles, features, 10)
                users = list(similar_users.User_Id)

                # taking acitivies
                results = self.recent_activity[self.recent_activity.User_Id.isin(
                    users)]

                # selecting those which are not reviewed by user
                ...
                results = results.filter(
                    ['Meal_Id', 'Name', 'catagory', 'Nutrient', 'Veg_Non', 'description', 'Price', 'Review'])
            ...
            def recent_activity_based(self, user_id):
                ...
                recent_data = self.df[self.df.Meal_Id.isin(
                    meal_ids)][['Nutrient', 'catagory', 'Disease', 'Diet']].reset_index(drop=True)
                ...
                features = list(
                    value_counts[recent_data.Nutrient.value_counts() > m].index)
