...
def signup_user(request):
    if request.method == 'POST':
        ...
        if len(fname) < 3 or fname.isnumeric():
            messages.error(
                request, "First Name should be string with more than 2 character")
            return render(request, 'website/signup.html')
        if len(lname) < 3 or lname.isnumeric():
            messages.error(
                request, "Last Name should be string with more than 2 character")
            return render(request, 'website/signup.html')
        if len(passw) < 5:
            messages.error(
                request, 'Length of password must be greater or equal to 5')
            return render(request, 'website/signup.html')
        if passw.isalnum() == False:
            ...
            try:
                ...
                if(myuser.username == number):
                    messages.error(request, ' Number :- '+myuser.username +
                                   ' already exist ! Please use another number')
                    return render(request, 'website/signup.html')
                ...
                login(request, user)
                messages.success(
                    request, "User created successfully, Now please complete your profile")
...
def fill_CSV(user, lst):
    ...
    if user in df.values:  # user is variable for user
        ...
        df = df.reset_index()
        # lst = values in list which we want to insert
...
def create_profile(request):
    if request.method == 'POST' and request.FILES['image']:
        ...
        prfl = Profile(name=name, email=email, number=number, gender=gender, age=age, blood=blood, weight=weight, height=height, favfood=favfood, foodtype=foodtype, diet=diet, nutrient=nutrient,
                       cuisines=cuisines, disease=disease, medicalhistory=medicalhistory, image=image)
        prfl.save()

        fill_CSV(request.user.username, [request.user.username, " ".join(ft), nutrient.replace(
            "++", " "), disease.replace("++", " "), diet.replace("++", " ")])
